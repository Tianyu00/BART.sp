...
)
{
   ...
   Rcpp::NumericMatrix Rw(_iw);
   int nrow = Rw.nrow();
   int k = Rw.ncol();
   arma::mat w(Rw.begin(), nrow, k, false);

   Rcpp::NumericVector Rtau2(_itau2s);
   arma::vec tau2(Rtau2.begin(), Rtau2.length());

   Rcpp::NumericVector Rrange(_iranges);
   arma::vec range(Rrange.begin(), Rrange.length());

   Rcpp::NumericVector Rsmoothness(_ismoothnesses);
   arma::vec smoothness(Rsmoothness.begin(), Rsmoothness.length());

   Rcpp::NumericMatrix Rdistance_all(_idistance_all);
   nrow = Rdistance_all.nrow();
   k = Rdistance_all.ncol();
   arma::mat ddistance_all(Rdistance_all.begin(), nrow, k, false);

   size_t nn2 = Rcpp::as<int>(_inn2);
   size_t nn = Rcpp::as<int>(_inn);
...
#pragma omp parallel num_threads(tc)
   ...
   if (new_loc){
      for(int i=0;i<=nd-1;i++) {
         ...
         covparms(0) = tau2(i);
         covparms(1) = range(i);
         covparms(2) = smoothness(i);
         S = MaternFun(ddistance_all, covparms);
         S11 = S.submat(0,0,nn2-1,nn2-1);
         S12 = S.submat(0,nn2,nn2-1,nn-1);
         S22inv = arma::inv(S.submat(nn2,nn2,nn-1,nn-1));
         VVV2 = S11 - S12 * S22inv * (S12.t());
         MMM = S12 * S22inv * w.row(i).t();
         w_test.row(i) = mvrnormArma(1, MMM, VVV2).row(0);
      }
   }
   ...
   Rcpp::List ret;
   ret["yhat.test"] = yhat;
   ret["w_test"] = Rcpp::NumericMatrix(Rcpp::wrap(w_test));
   return ret;
}
void getpred(int beg, int end, size_t p, size_t m, size_t np, xinfo& xi, std::vector<vtree>& tmat, double *px, Rcpp::NumericMatrix& yhat)
{
